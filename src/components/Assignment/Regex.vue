  ...
  <div>
    <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded" :class="[color === 'light' ? 'bg-white' : 'bg-emerald-900 text-white']">
      ...
      <div class="block w-full overflow-x-auto">
        ...
        <table class="table-fixed items-center w-full bg-transparent border-collapse">
          <thead>
            <tr>
              <th class="w-1/2 pl-4 align-middle border border-solid py-3 text-xs uppercase whitespace-nowrap font-semibold text-left" :class="[color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-emerald-800 text-emerald-300 border-emerald-700']">Question</th>
              <th class="align-middle border border-solid py-3 text-xs uppercase whitespace-nowrap font-semibold text-left" :class="[color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-emerald-800 text-emerald-300 border-emerald-700']">Marks</th>
              <th class="align-middle border border-solid py-3 text-xs uppercase whitespace-nowrap font-semibold text-left" :class="[color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-emerald-800 text-emerald-300 border-emerald-700']">Completion</th>
              <th class="pr-4 align-right border border-solid py-3 text-xs uppercase whitespace-nowrap font-semibold text-right" :class="[color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-emerald-800 text-emerald-300 border-emerald-700']">Write Pattern</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in questions" :key="index">
              <th class="border-t-0 align-middle border-l-0 border-r-0 text-xs p-4 text-left flex items-center">
                ...
                <span class="ml-1" :class="[color === 'light' ? 'text-blueGray-600' : 'text-white']">
                    {{ item.question }} <br> 
                    <span class="text-xs font-semibold inline-block py-1 px-2 rounded text-emerald-600 bg-emerald-200 last:mr-0 mr-1">{{ item.sentence }}</span>
                </span>
              </th>
              <td class="border-t-0 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{{ item.marks }} marks</td>
    ...
    <div v-if="loading">
      ...
      <div class="w-full h-full fixed block top-0 left-0 bg-white opacity-75 z-50">
        <span class="bg-emerald-500 opacity-75 top-1/2 my-0 mx-auto block relative w-0 h-0" style="top: 50%; left: 50%">
...
export default {
  ...
  methods: {
    async submit() {
      ...
      const req = await fetch("https://einfach.api.stdlib.com/application@dev/Regex/regexSubmit/", {
        method: "post",
    ...
    async fetchRTDB() {
      if (CryptoJS.AES.decrypt(VueCookies.get("fbb3cu24"), "736b9960-fbb3-4430-a653-f9f4d58ddfe1").toString(CryptoJS.enc.Utf8) && CryptoJS.AES.decrypt(VueCookies.get("fbb3em24"), "736b9960-fbb3-4430-a653-f9f4d58ddfe1").toString(CryptoJS.enc.Utf8)) {
        ...
        const regexIndex = snapshotData.data().assignment.tags.regex
        const req = await firebase.database().ref("regex").get()
        const regexDatabase = await req.val()
        const finalSet = {}
        regexIndex.forEach((e) => {
          finalSet[e.index] = regexDatabase[e.index]
